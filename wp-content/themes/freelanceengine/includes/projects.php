...
class Fre_ProjectAction extends AE_PostAction {
	...
	function post_sync() {
		...
		if( isset( $request['et_payment_package'] )  ){
			...
			$sku = $request['et_payment_package'];

			$can_post_free =  can_post_project_free($sku);
			//$can_post_free = true;
			//AE_Package::can_post_free($sku);
			if( ! $can_post_free ){
		...
		if ( ! is_wp_error( $result ) ) {
			...
			if ( isset( $request['et_payment_package'] ) ) {
				...
				et_track_payment('check package is purchased or not.');
				$pack_sku = $request['et_payment_package'];
				if(  is_acti_fre_membership() ){
					$check = apply_filters('package_or_free', $check, $pack_sku, $result);
				} else {
					$check = AE_Package::package_or_free( $pack_sku, $result );
				}
				...
				// this code might not available in new version. Note at Sep 2019.

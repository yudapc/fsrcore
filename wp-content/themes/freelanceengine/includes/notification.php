...
class Fre_Notification extends AE_PostAction {
	...
	function __construct() {
		...
		/**
		 * @since 1.8.8
		 * send notification to admin in some cases.
		*/
		$this->add_action( 'ae_after_change_status_publish', 'change_status_project_noti_onsite' );
		$this->add_action( 'ae_after_change_status_archive', 'change_status_project_noti_onsite');
	...
	function fre_notify_item( $notify ) {
		...
		switch ( $type ) {
			...
			case 'new_bid':
				...
				$message    = sprintf( __( '%s bid on your project %s', ET_DOMAIN ),
					'<strong>' . get_the_author_meta( 'display_name', $bid_author ) . '</strong>',
					'<strong>' . get_the_title( $project ) . '</strong>'
				);
				$content    .= '<a class="fre-notify-wrap" href="' . get_permalink( $project ) . '">
			...
			case 'bid_accept':
				...
				$project_owner = get_post_field( 'post_author', $project );
				$message       = sprintf( __( '%s accepted your bid on the project %s', ET_DOMAIN ),
					'<strong>' . get_the_author_meta( 'display_name', $project_owner ) . '</strong>',
					'<strong>' . get_the_title( $project ) . '</strong>'
				);
				$content       .= '<a class="fre-notify-wrap" href="' . get_permalink( $project ) . '">
			...
			case 'complete_project':
				...
				$project_owner = get_post_field( 'post_author', $project );
				$message       = sprintf( __( '%s finished your working project %s', ET_DOMAIN ),
					'<strong>' . get_the_author_meta( 'display_name', $project_owner ) . '</strong>',
					'<strong>' . get_the_title( $project ) . '</strong>'
				);
			...
			case 'project_status': // 1.8.8
				...
				$message        = sprintf( __( 'Your project has been published. Go to project details.', ET_DOMAIN ),
					'<strong>' . get_the_author_meta( 'display_name', $owner ) . '</strong>'	);
				if($status == 'archive'){
					$message        = sprintf( __( 'Your project has been archived. Go to project details.', ET_DOMAIN ),
					'<strong>' . get_the_author_meta( 'display_name', $owner ) . '</strong>'	);
				}
	...
	/**
	 * Add a notification onsite to admin.
	 * @since 1.8.8
	 * @author: danng
	*/
	function get_email_admin(){
		return ae_get_option( 'new_post_alert', '' ) ? ae_get_option( 'new_post_alert', '' ) : get_option( 'admin_email' );
	}
	function new_payment_notification_onsite($data){
		global $user_ID;
		$admin_email = $this->get_email_admin();
		$user = get_user_by('email', $admin_email);

		if( $user && ! is_wp_error($user) ){
			$content      = 'type=new_payment&buyer=' . $user_ID;
			$title        = __( 'New payment on site.' );
			$notification = array(
				'post_type'    => $this->post_type,
				'post_content' => $content,
				'post_excerpt' => $content,
	...
	/**
	 * send notification to employer/owner after his project is published.
	 * @since: 1.8.8
	*/
	function change_status_project_noti_onsite($post){
		...
		if( $owner && ! is_wp_error($owner) ){
			$content      = "type=project_status&status={$post->post_status}&project={$post->ID}&owner=" . $owner->ID;
			$title        = __( 'Your project is publish.' );
			if($post->post_type == 'archive')
				$title        = __( 'Your project is archived.' );
			$notification = array(
				'post_type'    => $this->post_type,
				'post_content' => $content,
				'post_excerpt' => $content,
...
global $et_notification;

$et_notification = new Fre_Notification();

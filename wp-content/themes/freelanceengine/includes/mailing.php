...
Class Fre_Mailing extends AE_Mailing {
	...
	function review_resubmmit_mail( $post_id ) {
		...
		$send_mail = ae_get_option('opt_resubmitted_project_email');
		if( $send_mail ){ // @since 1.8.8
			...
			$subject = __( "A new re-submitted job.", ET_DOMAIN );
	 ...
	 */
	function new_payment_notification_mail( $order_id ) {
		$mail    = ae_get_option( 'new_post_alert', '' ) ? ae_get_option( 'new_post_alert', '' ) : get_option( 'admin_email' );
		$subject = __( "A new payment notification.", ET_DOMAIN );

		$opt_send = ae_get_option( 'opt_new_payment_mail', true);
	...
	function bid_cancel_mail( $project_id ) {
		...
		if ( $author ) {
			...
			$subject = sprintf( __( "A talent withdrew from your job listing: %s.", ET_DOMAIN ), get_option( 'blogname' ) );
			$opt_send = ae_get_option('opt_bid_cancel_mail', true);
			if($opt_send){
				return $this->wp_mail( $author->user_email, $subject, $message, array(
					'post'    => $project_id,
					'user_id' => $post_author
	...
	function review_freelancer_email( $project_id ) {
		...
		$subject = __( "Your job has been finished.", ET_DOMAIN );
		...
		$opt_send = ae_get_option('opt_complete_project_mail', true);
	...
	function review_employer_email( $project_id ) {
		...
		$subject = __( "A new review from talent.", ET_DOMAIN );
	...
	function bid_accepted( $freelancer_id, $project_id ) {
		...
		$subject = sprintf( __( "Your interest on job %s has been accepted.", ET_DOMAIN ), get_the_title( $project_id ) );
	...
	function bid_accepted_alternative( $freelancer_id, $project_id ) {
		...
		if ( $q_bid->have_posts() ) {
			...
			$subject = __( "Thank you for submitting your interest.", ET_DOMAIN );
	...
	function new_message( $receiver, $project, $message ) {
		...
		// send mail
		$opt_send = ae_get_option('opt_new_msg_email', true);
		if($opt_send) {
	...
	function new_report( $project_id, $report ) {
		...
		$subject = sprintf( __( "Have a new report on job %s.", ET_DOMAIN ), get_the_title( $project_id ) );
		$opt_send = true;
		if ( $project->post_author == $user_ID ) {
			...
			$receiver   = $bid_author;
			$opt_send = ae_get_option('opt_employer_report_mail', true);
		} else {
			...
			$opt_send = ae_get_option('opt_freelancer_report_mail', true);
		}
		...
		$opt_send = ae_get_option( 'opt_admin_report_email', true);
		if( $opt_send ){
	...
	/**
	 * send email to freelancer, admin after employer request close project
	 *
	 * @param snippet
	 *
	 * @since 1.3
	...
	function close_project( $project_id, $message ) {
		...
		$subject_freelancer       = __( "A new dispute on your working job", ET_DOMAIN );
		...
		$subject_admin       = __( 'A new dispute job on your site', ET_DOMAIN );


		$opt_send = ae_get_option( 'opt_employer_close_mail_template', true); //ok
		if($opt_send){
			// send mail to admin
	...
	function quit_project( $project_id, $message ) {
		...
		$subject_freelancer       = __( "A new dispute on your job", ET_DOMAIN );
		...
		$send_opt = ae_get_option('opt_freelancer_quit_mail'); //ok
		if($send_opt){
			$this->wp_mail( get_option( 'admin_email' ), $subject_admin, $mail_template_admin, array(
				'user_id' => 1,
				'post'    => $project_id
	...
	function execute_payment( $project_id, $bid_accepted ) {
		...
		$subject_employer       = __( 'Final result of the dispute on your job', ET_DOMAIN );
		...
		$subject_freelancer = __( 'Final result of the dispute on your working job', ET_DOMAIN );
	...
	function notify_execute( $project_id, $bid_accepted ) {
		...
		$subject_employer       = __( 'Your job is completed', ET_DOMAIN );
	...
	function new_project_of_category( $project ) {
		...
		if(!empty($project->tax_input['skill'])){
			...
			$subject   = __( 'New job For You Today!', ET_DOMAIN );
			$headers[] = 'Content-Type: text/html; charset=UTF-8';
			$headers[] = 'From: ' . get_option( 'blogname' ) . ' <' . get_option( 'admin_email' ) . '>' . "\r\n";
...
if ( ! function_exists( 'send_receipt_mail' ) ) {
	...
	function send_receipt_mail( $content, $order ) {
		...
		$content 	= apply_filters( 'ae_send_receipt_credit_mail', $content,  $pack, $type, $order); // add from 1.8.6
		...
		if ( $order['payment'] == 'cash' ) {
			...
			if ( $type == 'bid_plan' ) {
				...
				$content = str_ireplace( '[notify_cash]', __( 'Please send the payment to admin to complete your payment.<br>Your job post is under admin review. It will be active right after admin approval.', ET_DOMAIN ), $content );
			}
		} else {

...
class Fre_BidAction extends AE_PostAction {
	...
	/**
	 * Only Send email to admin when a freelancer buy a bid package.
	 * No have notificaton onsite
	 */
	public function fre_notify_admin_bid( $data ) {
		if ( ! isset( $data['order_id'] ) ) {
			return false;
		}
		$this->mail->new_payment_notification_mail( $data['order_id'] );
		Fre_Notification::getInstance()->new_payment_notification_onsite($data);
	}
	...
	function ae_convert_bid( $result ) {
		...
		if ( $project_post && ! is_wp_error( $project_post ) ) {
			...
			$result->total_bids   = $result->bid_average = 0;
			$result->project_status_view = '';
		}

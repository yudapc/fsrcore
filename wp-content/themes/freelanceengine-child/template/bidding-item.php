            ...
            <div class="col-md-4 col-sm-12"> <?php
            	...
            	if( $show_bid_info ){ ?>
	                <div class="col-free-reputation">	
						...
						printf( __( '<p>%s job(s) worked</p>', ET_DOMAIN ), $convert->total_projects_worked );
						?>
	                </div> <?php
		        } else {
		        	echo '<span class="msg-secret-bid">';
		        	_e('Only job owner can view this information.',ET_DOMAIN);
		        	echo '</span>';
		    	} ?>
    ...
    <div class="col-md-3 col-sm-3">
        <div class="col-free-bid">
        	...
        	if( $show_bid_info  ) { ?>
	            ...
	            <p class="hidden-lg hidden-md hidden-sm"><?php _e( 'Hourly Rate', ET_DOMAIN ); // @TODO: Make own entry. ?></p>
			...
			if ( $user_ID == $project->post_author ) {
				if ( $convert->flag == 1 ) {
					if ( ae_get_option( 'use_escrow' ) ) {
						//echo '<a id="' . get_the_ID() . '" rel="' . $project->ID . '" class="fre-normal-btn btn-accept-bid">' . __( 'Accept Interview', ET_DOMAIN ) . '</a>';
						echo '<a class="fre-normal-btn btn-accept-bid btn-accept-bid-no-escrow" id="' . get_the_ID() . '">' . __( 'Hire', ET_DOMAIN ) . '</a>';
					} else {
						echo '<a class="fre-normal-btn btn-accept-bid btn-accept-bid-no-escrow" id="' . get_the_ID() . '">' . __( 'Hire', ET_DOMAIN ) . '</a>';
					}
				}
